...
export default function BateauDetailPage({ route }) {
  ...
  useEffect(() => {
    const fetchBateau = async () => {
      try {
        const response = await fetch(`https://glorious-system-vgg4vv7vp773wjvw-8000.app.github.dev/api/bateau/${bateauId}`);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Image
        source={{ uri: `https://glorious-system-vgg4vv7vp773wjvw-8000.app.github.dev/storage/images/bateaux/${bateau.photo}` }}
        style={styles.photo}
      />
