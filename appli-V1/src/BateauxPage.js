...
export default function BateauxPage() {
  ...
  useEffect(() => {
    const fetchBateaux = async () => {
      try {
        const response = await fetch('https://glorious-system-vgg4vv7vp773wjvw-8000.app.github.dev/api/bateaux');
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => navigation.navigate('BateauDetails', { bateau: item.id })}>
      <View style={styles.card}>
        <Image
          source={{ uri: `https://glorious-system-vgg4vv7vp773wjvw-8000.app.github.dev/storage/images/bateaux/${item.photo}` }}
          style={styles.photo}
        />
